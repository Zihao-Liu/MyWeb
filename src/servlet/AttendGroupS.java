...
public class AttendGroupS extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(attendGroupDao.findAttendGroup(user.getUserID(), groupID)==null){
			groupDao.addGroupUserNum(groupID);
			userDao.addUserAttend(user.getUserID());
			attendGroupDao.attend(user.getUserID(), groupID);
			request.setAttribute("attendflag",attendGroupDao.findAttendGroup(user.getUserID(), groupID));
		}
		else{
			request.setAttribute("error", "ƒ˙“—º”»Î");
		}
